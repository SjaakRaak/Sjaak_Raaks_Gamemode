GM.Name = "Sjaak Raak's Gamemode"
GM.Author = "Sjaak_Raak"
...
GM.TeamBased = true -- false blokkeert ook het menu, maar is wel lelijk - spectator nog zichtbaar. Beter proberen om de menuknop uit te zetten in maps zonder teamplay.
...
function GM:CreateTeams()
	...
	team.SetUp( blue_team, "Blue team", Color( 7, 64, 112 ) )
	team.SetSpawnPoint( blue_team, "info_player_combine", true )
	
	team.SetUp( red_team, "Red team", Color( 116, 17, 22 ) )
	team.SetSpawnPoint( red_team, "info_player_rebel", true )
...
RunConsoleCommand( 'phys_penetration_error_time', '0' )
RunConsoleCommand( 'mp_falldamage', '1' )
RunConsoleCommand( 'sv_footsteps', '1' )
RunConsoleCommand( 'sk_healthkit', '40' ) -- standaard 25
RunConsoleCommand( 'sk_battery', '20' ) -- standaard 15

-- geen effect
RunConsoleCommand( 'mp_teamplay', '1' ) -- teamplay lijkt ander systeem te gebruiken dan deze, geen effect op menu of teams
RunConsoleCommand( 'mp_allowspectators', '0' )  -- zelfde, geen effect op wel/geen spectator kunnen zijn
RunConsoleCommand( 'cl_defaultweapon', 'weapon_frag' )
RunConsoleCommand( 'mp_forcerespawn', '0' )
RunConsoleCommand( 'mp_footsteps', '1' )

-- blocked
// RunConsoleCommand( "sv_cheats", "1" ) -- werkt niet, is geblokkeerd door valve. kan je alleen handmatig in het level invoeren.
// RunConsoleCommand( 'mp_flashlight', '1' ) -- onzin: command werkt gewoon (flashlight wel/niet aan), maar geeft ook lua_error dat het command geblockt is. Staat gelukkig ook bij player_stats.
// RunConsoleCommand( 'hud_quickinfo', '0' )


-- Zombies, deze werken!!! yessss

-- Headcrabs
RunConsoleCommand( 'sk_headcrab_health', '15' )
RunConsoleCommand( "sk_headcrab_fast_health", "10" )
RunConsoleCommand( "sk_headcrab_poison_health", "20" )
RunConsoleCommand( "sk_headcrab_melee_dmg", "24" )

-- Zombies
RunConsoleCommand( "sk_zombie_health", "100" )
RunConsoleCommand( "sk_zombie_poison_health", "200" )
RunConsoleCommand( "sk_zombie_soldier_health", "150" )
RunConsoleCommand( "sk_zombie_dmg_one_slash", "34" )
RunConsoleCommand( "sk_zombie_dmg_both_slash", "44" )
RunConsoleCommand( "sk_zombie_poison_dmg_spit", "24" )
...
local zoom_spread = CreateConVar("sv_ehw_zoom_spread_mult", 1, {FCVAR_ARCHIVE, FCVAR_GAMEDLL})
...
// Autostart Enhanced Playermodel Selector
// Credits Woflje



// hook.Add( "InitPostEntity", "join_con_commands", function() -- functie verwijderd
//    RunConsoleCommand( "playermodel_selector" )
// end )

hook.Remove( "InitPostEntity", "join_con_commands", function() -- deze moet wel! anders reset dit bij opstarten nieuw spel
	RunConsoleCommand( "playermodel_selector" )
end )
...
function Menu.Setup()
	...
	local fw, fh = math.min( ScrW() - 16, 920 ), math.min( ScrH() - 16, 660 )
	Frame:SetSize( fw, fh )
	Frame:SetTitle( "Player model setup (F1)" )	
	Frame:SetVisible( true )
	Frame:SetDraggable( true )
	Frame:SetScreenLock( false )
	Frame:ShowCloseButton( true )  -- DIT IS/ZIJN DE DRIE KNOPJES RECHTSBOVEN
	Frame:Center()
	Frame:MakePopup()
	Frame:SetKeyboardInputEnabled( false )
	...
	Menu.ResetButton:SetSize( 120, 30 )
	Menu.ResetButton:SetPos( 5, fh - 35 )
	Menu.ResetButton:SetText( "Apply" )
	Menu.ResetButton.DoClick = mdl.DefaultPos
	Menu.ResetButton.DoClick = LoadPlayerModel
